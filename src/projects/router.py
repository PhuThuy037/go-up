...
from src.projects.response import ProjectWithTasks, ProjectWithUsers
...
def create_new_project(
    ...
    return project_services.create_project(
        session=session, project=project, owner_id=current_user.id
    )
...
@router.get("/{project_id}/tasks", response_model=ProjectWithTasks)
def get_project_tasks(session: SessionDep, project_id: int):
    return project_services.get_project_tasks(session=session, project_id=project_id)


@router.get("/{project_id}/members", response_model=ProjectWithUsers)
def get_project_members(session: SessionDep, project_id: int):
    return project_services.get_all_member(session=session, project_id=project_id)
