...
from src.tasks.utils import check_project_exists


def create_project(
    *, session: Session, project: ProjectCreate, owner_id: int
) -> Project:
...
def update_project(
    *, session: Session, project_id: int, project: ProjectUpdate, current_user_id: int
) -> Project:
    ...
    if project_exist.owner_id != current_user_id:
        raise AppError(
            403,
            "FORBIDDEN",
            "Only owner can update this project",
            details=[{"field": "owner_id", "reason": "not_owner"}],
        )
    project_exist.make(project)
    session.commit()
    session.refresh(project_exist)
    return project_exist


def get_project_tasks(*, session: Session, project_id: int) -> list[Project]:
    check_project_exists(session=session, project_id=project_id)
    return session.exec(
        select(Project)
        .where(Project.id == project_id)
...
def get_all_member(*, session: Session, project_id: int) -> Project:
    check_project_exists(session=session, project_id=project_id)
    return session.exec(
        select(Project)
        .where(Project.id == project_id)
